...
#ifndef INCLUDE_VDA5050_NODEPOSITION_H_
#define INCLUDE_VDA5050_NODEPOSITION_H_
...
struct NodePosition {
  ...
  inline bool operator==(const NodePosition &other) const {
    if (this->allowedDeviationTheta != other.allowedDeviationTheta) return false;
    if (this->allowedDeviationXY != other.allowedDeviationXY) return false;
  ...
  ///\return is not equal?
  ///
  inline bool operator!=(const NodePosition &other) const { return !this->operator==(other); }
};
...
#endif  // INCLUDE_VDA5050_NODEPOSITION_H_
